...
public:
    L3Address ipAddr;
    std::list<simtime_t> sentPcktListTime;
...
public:
   ...
   // std::vector<std::tuple<L3Address, double, double, long>> possibleGateways; // <address, sinr, rssi> <address, sinr, rssi, droppedDCCount>
    std::vector<std::tuple<L3Address, double, double, simtime_t>> possibleGateways; // <address, sinr, rssi> <address, sinr, rssi, droppedDCCount>
...
class INET_API NetworkServerApp : public cSimpleModule, cListener
{
  ...
  protected:
    ...
    void sendAck(LoRaMacFrame* pkt, L3Address pickedGateway, double SNIRinGW, double RSSIinGW);
    void receiveSignal(cComponent *source, simsignal_t signalID, long value, cObject *details) override;
    bool evaluateADRinServer;
    bool confirmedTrafficMode;
    cHistogram receivedRSSI;
    void updateKnownGateways(LoRaMacFrame* pkt);
