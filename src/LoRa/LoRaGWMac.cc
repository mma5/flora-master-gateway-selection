...
void LoRaGWMac::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
      ...
      //  LoRa_GW_PacketDroppedSignal = registerSignal("LoRa_GW_PacketDropped");
...
void LoRaGWMac::handleUpperPacket(cPacket *msg)
{
    if(waitingForDC == false)
    {
        ...
        waitingForDC = true;
       // double delta;
        if(frame->getLoRaSF() == 7) delta = 5.7; //0.61696;// 5.7; // 0.61696;
        if(frame->getLoRaSF() == 8) delta = 10.3;//1.23392;//10.3; //1.23392;
        if(frame->getLoRaSF() == 9) delta = 20.6;//2.14016;//20.6; //2.14016;
        if(frame->getLoRaSF() == 10) delta = 37.1;//4.28032;//37.1; //4.28032;
        if(frame->getLoRaSF() == 11) delta = 74.1; //7.24992;//74.1; //7.24992;
        if(frame->getLoRaSF() == 12) delta = 148.3;//14.49984;//148.3; //14.49984;
        scheduleAt(simTime() + delta, dutyCycleTimer);
        GW_forwardedDown++;
        endOfWaitungDC= simTime() + delta;
    }
    else
    {
        if (simTime() >= getSimulation()->getWarmupPeriod()){
        GW_droppedDC++;
        GW_droppedDCListTime.push_back(simTime());}
       // EV<<"EMITTING LoRa_GW_PacketDroppedSignal"<<endl;
       // emit(LoRa_GW_PacketDroppedSignal, 50);
        //GW_droppedDC
        delete msg;
    }
}
...
//getGW_droppedDC
simtime_t LoRaGWMac::getGW_droppedDC()
{
   /* int  counterfordroppedDCInThisGatewayForLastInterval=0;
    for (std::list<simtime_t>::iterator it=GW_droppedDCListTime.begin(); it != GW_droppedDCListTime.end(); ++it)
        {
         if (*it > (simTime()-delta)){
             counterfordroppedDCInThisGatewayForLastInterval++;
                                    }
        }*/
    return endOfWaitungDC;//(long)counterfordroppedDCInThisGatewayForLastInterval;
}
}
