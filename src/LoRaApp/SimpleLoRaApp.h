...
class INET_API SimpleLoRaApp : public cSimpleModule, public ILifecycle
{
    protected:
        ...
        int sentPackets;
        int sentPacketsWithRetransmsionSimpleLoraApp;

        int receivedADRCommands;
        int receivedAckMsg;
        int sentUniqueMsg;
        int givenUpSimpleLoRa;
        double totalRetries;

        int lastSentMeasurement;
        simtime_t timeToFirstPacket;
        simtime_t timeToNextPacket;
        simtime_t ackTimeoutSimpleLoRaApp = -1;
        int sequenceNumberSimpleLoRaApp = 0;
        bool msgIsRetransmitAck;
        int retryLimit = -1;
        /** Number of frame retransmission attempts. */
            int retryCounter = -1;
            int valueOfSequenceNumber;
